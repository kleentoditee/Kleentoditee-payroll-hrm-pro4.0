...
export default function EditEmployeePage() {
  ...
  const loadEmployee = useCallback(async () => {
    ...
    setNotes(e.notes);
    setActive(e.active);
    setPayrollTaxExemptionEnabled(e.payrollTaxExemptionEnabled ?? true);
    ...
    setWorkAuthorizationStatus(e.workAuthorizationStatus ?? "NOT_SPECIFIED");
  ...
  async function onUpload(file: File, type: string): Promise<boolean> {
    setError(null);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const body: Record<string, unknown> = {
        ...
        employmentEndDate: employmentStatus === "ended" ? empEnd || null : null,
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4">
      ...
      <div className="overflow-x-auto" role="tablist" aria-label="Employee form sections">
        <div className="flex min-w-max gap-2 border-b border-slate-200 pb-2">
          {tabs.map((item) => (
            <button
              key={item.id}
              type="button"
              ...
              className={`rounded-lg px-3 py-2 text-sm font-semibold ${
                tab === item.id ? "bg-brand text-white" : "text-slate-600 hover:bg-slate-100"
              }`}
            >
              {item.label}
            </button>
      ...
      {tab === "profile" ? (
        <section className="flex flex-col gap-4 rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center">
        <EmployeeAvatar
          key={avatarTick}
          employeeId={id}
          hasPhoto={hasPhoto}
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">Choose a PNG, JPG, or WebP image, then position and zoom it.</p>
          <input
            type="file"
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        {tab === "profile" ? (
          ...
          <label className="block text-sm">
            <span className="text-slate-700">Sex (government forms)</span>
            <select value={sex} onChange={(e) => setSex(e.target.value as typeof sex)} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2">
          ...
          <label className="block text-sm">
            ...
            <div className="mt-1 flex gap-2">
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="min-w-0 flex-1 rounded-lg border border-slate-200 px-3 py-2" />
              {email ? <a href={`mailto:${email}`} className="rounded-lg border border-slate-200 px-3 py-2 font-semibold text-brand hover:bg-slate-50">Send</a> : null}
            </div>
          </label>
          </section>
        ) : null}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="block text-sm">
              ...
              <select
                ...
                onChange={(e) => {
                  const status = e.target.value as "current" | "ended";
                  setEmploymentStatus(status);
                  if (status === "current") setEmpEnd("");
            ...
            {employmentStatus === "ended" ? (
              <label className="block text-sm">
                <span className="text-slate-700">Employment end date</span>
                <input required type="date" value={empEnd} onChange={(e) => setEmpEnd(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2" />
              </label>
            ) : null}
            <label className="block text-sm">
              <span className="text-slate-700">Immigration / work status</span>
              <select
                value={workAuthorizationStatus}
                onChange={(e) => {
                  const status = e.target.value as WorkAuthorizationStatus;
                  setWorkAuthorizationStatus(status);
                  if (status !== "WORK_PERMIT") {
                    setWorkPermit("");
                    setWpExp("");
                  }
                }}
            ...
            {workAuthorizationStatus === "WORK_PERMIT" ? (
              <>
                ...
                <label className="block text-sm">
                  ...
                  <input required type="date" value={wpExp} onChange={(e) => setWpExp(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2" />
                </label>
              </>
            ) : null}
          </div>
        ...
        {tab === "documents" ? (
          ...
          <p className="text-xs text-slate-500" key={docTick}>Upload PDF, PNG, JPG, or WebP files. Identity documents are restricted to authorized HR and payroll roles.</p>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-4">
          <div className="flex gap-2">
            <button type="button" disabled={tab === tabs[0].id} onClick={() => moveTab(-1)} className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold disabled:opacity-40">
              Back
            </button>
            <button type="button" disabled={tab === tabs[tabs.length - 1].id} onClick={() => moveTab(1)} className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold disabled:opacity-40">
              Next
            </button>
          </div>
          {tab !== "access" ? (
      ...
      {photoToCrop ? (
        <ProfilePhotoEditor
          ...
          onSave={async (croppedFile) => {
            const saved = await onUpload(croppedFile, "PHOTO");
            if (saved) setPhotoToCrop(null);
            return saved;
          }}
        />
      ) : null}
