  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const tj = (await tRes.json()) as { items: Tpl[] };
        const ej = (await eRes.json()) as { entry: Entry & { employee: { fullName: string } } };
        if (cancelled) {
          return;
        }
        ...
        setEmployeeName(ej.entry.employee.fullName);
        setMonth(e.month);
        setSite(e.site);
        setStatus(e.status);
        setDaysWorked(String(e.daysWorked));
        setHoursWorked(String(e.hoursWorked));
        setOvertimeHours(String(e.overtimeHours));
        setFlatGross(String(e.flatGross));
        setBonus(String(e.bonus));
        setAllowance(String(e.allowance));
        setAdvanceDeduction(String(e.advanceDeduction));
        setWithdrawalDeduction(String(e.withdrawalDeduction));
        setLoanDeduction(String(e.loanDeduction));
        setOtherDeduction(String(e.otherDeduction));
        setTemplateId(e.templateId);
        setApplyNhi(e.applyNhi);
        setApplySsb(e.applySsb);
        setApplyIncomeTax(e.applyIncomeTax);
        setNotes(e.notes);
        setError(null);
      } catch {
        if (!cancelled) {
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr,280px]">
        <form onSubmit={onSubmit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-xs text-slate-500">Employee is fixed on this row; create a new timesheet to use another person.</p>
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block text-sm">
              <span className="text-slate-700">Month</span>
              <input type="month" required value={month} onChange={(e) => setMonth(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2" />
            </label>
            <label className="block text-sm">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button type="button" onClick={onDelete} className="rounded-lg border border-red-300 px-4 py-2 text-sm font-semibold text-red-700 hover:bg-red-50">
              Delete
            </button>
          </div>
        </form>
