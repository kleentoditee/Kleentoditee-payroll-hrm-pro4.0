...
export default function NewBillPage() {
      ...
      rows.map((r) =>
        r.key !== key
          ...
          : {
              ...
              unitCost: product ? String(product.purchaseCost) : r.unitCost,
              expenseAccountId: product?.expenseAccountId ?? r.expenseAccountId
            }
      )
    );
  }
  ...
  return (
    <div className="space-y-6">
      <FinanceRecordBreadcrumbs recordLabel="New bill" />
      <div>
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        <section className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:grid-cols-3">
          ...
          <label className="text-sm">
            <span className="text-slate-700">Bill number</span>
            <input
              value={number}
          ...
          <label className="text-sm">
            <span className="text-slate-700">Bill date</span>
            <input
              type="date"
              required
              value={billDate}
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="space-y-3">
            {lines.map((line) => (
              <div
                key={line.key}
                className="grid min-w-0 gap-2 rounded-xl border border-slate-100 bg-slate-50 p-3 md:grid-cols-[2fr_3fr_6rem_8rem_2fr_auto]"
              >
                <select
        ...
        <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          <button
            type="button"
            onClick={() => router.push("/dashboard/finance/bills")}
