            ...
            <article key={row.id} className="min-w-0 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <h3 className="break-words font-bold text-slate-950">{row.displayName}</h3>
                  <p className="mt-1 break-words text-sm text-slate-600">{row.companyName || <MissingValue />}</p>
                </div>
                <span className={`shrink-0 rounded-full px-2 py-1 text-xs font-bold ${row.active ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-600"}`}>
              ...
              <dl className="mt-3 grid gap-2 text-sm">
                <div className="flex min-w-0 justify-between gap-3">
                  ...
                  <dd className="min-w-0 break-words text-right font-semibold text-slate-900">{row.primaryContact || <MissingValue />}</dd>
                </div>
                <div className="flex min-w-0 justify-between gap-3">
                  <dt className="shrink-0 text-slate-500">Phone</dt>
                  <dd className="min-w-0 break-words text-right font-semibold text-slate-900">{row.phone || <MissingValue />}</dd>
                </div>
                <div className="flex min-w-0 justify-between gap-3">
                  <dt className="shrink-0 text-slate-500">Email</dt>
                  <dd className="min-w-0 break-all text-right font-semibold text-slate-900">{row.email || <MissingValue />}</dd>
                </div>
                <div className="flex min-w-0 justify-between gap-3">
                  <dt className="shrink-0 text-slate-500">Open balance</dt>
                  <dd className="text-right font-semibold text-slate-900">
                    {formatMoney(row.openBalance ?? 0)}
                    {(row.billCount ?? 0) > 0 ? <span className="block text-xs font-normal text-slate-500">{row.billCount} open bill{row.billCount === 1 ? "" : "s"}</span> : null}
                  </dd>
                </div>
              </dl>
              <div className="mt-4 grid grid-cols-2 gap-2">
                <Link href="/dashboard/finance/bills/new" className="inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-300 px-3 py-2 text-center text-sm font-bold text-[#063E4A] outline-none ring-[#006D77] hover:bg-[#EAF6F7] focus-visible:ring-2">
                  Create bill
                </Link>
                <Link href="/dashboard/finance/bill-payments/new" className="inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-300 px-3 py-2 text-center text-sm font-bold text-[#063E4A] outline-none ring-[#006D77] hover:bg-[#EAF6F7] focus-visible:ring-2">
                  Record payment
                </Link>
              </div>
            </article>
